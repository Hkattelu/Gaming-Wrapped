...
describe('POST /api/igdb/cover', () => {
  ...
  it('400 when body.title is missing/invalid', async () => {
    ...
    const res = await POST({ json: async () => ({ title: '    ' }) } as unknown as Parameters<typeof POST>[0]);
  ...
  it('200 with { imageUrl } when resolver returns a URL', async () => {
    ...
    const { POST } = await import('./route');
    const res = await POST({ json: async () => ({ title: 'Halo' }) } as unknown as Parameters<typeof POST>[0]);
